...
hhOnly_df <- asec_clean %>% 
  ...
  dummy_cols(select_columns = c('whyptly', 'whynwly'), ignore_na = TRUE) %>% 
  mutate(#across((contains("whyptly_") & !matches("whyptly_0")), ~if_else(whyptly_0 == 1, NA, .x)),
         across((contains("whynwly") & !matches("whynwly_0")), ~if_else(whynwly_0 == 1, NA, .x))) %>% 
  select(-whyptly_0, -whynwly_0)
...
all_adults <- hhOnly_df %>% 
  ...
  mutate(
    ...
    full_emp = case_when(relate == 101 & wksunem1 == 0 ~ 1,
                          relate == 101 & wksunem1 > 0 & wksunem1 != 99 ~ 0,
                          TRUE ~ NA),
    ...
    across(offtotval | pov100 | pov50 | contains("whynwly_") | contains("whyptly_") | workly | wksunem1 | wksunem_int, ~if_else(relate == 101, .x, 0)),
  ...
  group_by(year, serial) %>%
  summarize(across(contains("_hh") | contains("wbhaa_") | offtotval | asecwth | pov100 | 
                     pov50 | spmpov | contains("whynwly_") | contains("whyptly_") | workly | wksunem1 | wksunem_int | full_emp, ~max(.x)),
            across(contains("earner") | (contains("ft") & !contains("totval")) | (contains("pt") & !contains("whyptly")) | adult, ~sum(.x, na.rm = TRUE))) %>% 
  ungroup() %>% 
  # at least one related adult or parent in household is an earner, working ft/pt
  mutate(across(contains("earner") | (contains("ft") & !contains("totval")) | (contains("pt") & !contains("whyptly")), ~if_else(.x >= 1, 1, 0), .names = "at1_{.col}"),
         ...
         wksunem1 = if_else(wksunem1 == 99, NA, wksunem1),
         wksunem_int = if_else(is.na(wksunem1) | wksunem1 == 0, NA, wksunem_int),
         fully_emp = if_else(wksunem1 == 0, 1, 0)) %>% 
...
lf_stats_all <- all_adults %>% 
  ...
  summarize_groups(wbhaa_white | wbhaa_black | wbhaa_hispanic | wbhaa_AIAN | wbhaa_AAPI,
                   ...
                   full_emp = weighted.mean(full_emp, w = asecwth, na.rm = TRUE),
                   wksunem_int = weighted.mean(wksunem_int, w = asecwth, na.rm = TRUE),
                   #whyptly_1 = weighted.mean(whyptly_1, w = asecwth, na.rm = TRUE),
                   #whyptly_2 = weighted.mean(whyptly_2, w = asecwth, na.rm = TRUE),
                   #whyptly_3 = weighted.mean(whyptly_3, w = asecwth, na.rm = TRUE),
                   #whyptly_4 = weighted.mean(whyptly_4, w = asecwth, na.rm = TRUE),
                   whynwly_1 = weighted.mean(whynwly_1, w = asecwth, na.rm = TRUE),
                   whynwly_2 = weighted.mean(whynwly_2, w = asecwth, na.rm = TRUE),
                   whynwly_3 = weighted.mean(whynwly_3, w = asecwth, na.rm = TRUE),
                   whynwly_4 = weighted.mean(whynwly_4, w = asecwth, na.rm = TRUE),
                   whynwly_5 = weighted.mean(whynwly_5, w = asecwth, na.rm = TRUE),
                   whynwly_7 = weighted.mean(whynwly_7, w = asecwth, na.rm = TRUE)) %>% 
  mutate(across(contains("adult") | contains("earner") | contains("at") | full_emp | wksunem_int | contains("why"), ~ round(.x, 3)),
  ...
  rename(Race = group_name,
         ...
         "At least 1 parent working part-time" = at1_pt_p, "At least 1 parent working full-time" = at1_ft_p,
         "Head 0 weeks unemployed" = full_emp, "Share of heads with long-term unemployment" = wksunem_int,
         #"Pt b/c could not find ft job" = whyptly_1, "Pt b/c wanted pt" = whyptly_2, "Pt b/c slack work" = whyptly_3, "Pt b/c other" = whyptly_4
         "Head didn't work; couldn't find work" = whynwly_1, "Head didn't work; ill or disabled" = whynwly_2, "Head didn't work; taking care of home/family" = whynwly_3,
         "Head didn't work; going to school" = whynwly_4, "Head didn't work; retired" = whynwly_5, "Head didn't work; other" = whynwly_7) %>% select(-group_value)
...
lf_stats_nmp <-  all_adults %>% 
  ...
  rename(Race = group_name,
         ...
         "Share with 3+ earners in household" = num_earner_3, "At least 2 adult earners in household" = at2_earner_adult,
         "At least 1 person working part-time" = at1_pt, "At least 1 person working full_time" = at1_ft, 
         "Parent is earners" = all_earner_p, "Parent is nonearner" = all_nonearner_p, 
         "Parent working part-time" = at1_pt_p, "Parent working full-time" = at1_ft_p,
         "Head 0 weeks unemployed" = full_emp, "Share of heads with long-term unemployment" = wksunem_int,
         #"Pt b/c could not find ft job" = whyptly_1, "Pt b/c wanted pt" = whyptly_2, "Pt b/c slack work" = whyptly_3, "Pt b/c other" = whyptly_4
         "Head didn't work; couldn't find work" = whynwly_1, "Head didn't work; ill or disabled" = whynwly_2, "Head didn't work; taking care of home/family" = whynwly_3,
         "Head didn't work; going to school" = whynwly_4, "Head didn't work; retired" = whynwly_5, "Head didn't work; other" = whynwly_7) %>% select(-group_value)
